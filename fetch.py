...
search, insert one batch via topics.py — and exits. One job runs at a time;
its stdout/stderr go to data/fetch.log so the UI can show progress and the
bot's final rundown.

Fetching only ever adds pending topics. Existing rows are never touched —
clearing the queue is what Decline is for.
...
FETCH_PROMPT = (
    ...
    "with `python3 topics.py add-batch`. Only add topics: never delete, "
    "modify, or re-status existing rows — old pending topics stay until a "
    "host declines them in the UI. Finish with a one-line-per-topic rundown "
    "of what you added."
)
...
def _count_topics():
    conn = db.connect()
    try:
        return conn.execute("SELECT COUNT(*) AS n FROM topics").fetchone()["n"]
    finally:
        conn.close()
...
def status():
    with _lock:
        snap = {k: v for k, v in _job.items() if k != "topics_before"}
...
def start():
    ...
    with _lock:
        ...
        _job.update(started_at=db.utcnow_iso(), finished_at=None,
                    returncode=None, added=None, topics_before=before)
        try:
            ...
            log.close()
            _job.update(state="error", finished_at=db.utcnow_iso(), added=0)
            return True
...
def _reap(proc, log):
    ...
    added = _count_topics()
    with _lock:
        added -= _job["topics_before"] or 0
        _job.update(finished_at=db.utcnow_iso(), returncode=rc, added=added)
